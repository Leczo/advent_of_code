...
type Board struct {
	content          []string // slice of numbers preserved as strings
	mappedContent    map[string]element
	selectedNumbers  [25]bool // 5x5 slice of possible to be selected values
...
func main() {
	...
	var winningNumberIndex int

	for _, boardPointer := range boards {
		for index, number := range drawnNumbers {
			...
			if isWinner {
				...
				if index > winningNumberIndex {
					...
					lastWinner = boardPointer
				}
				break
			}
		}
	}
	calculateResult(*lastWinner)
